...
const legalContent = createLegalContent({
  siteName: 'QuickTextFormatter',
  siteReference: 'www.quicktextformatter.com',
  contactEmail: 'contact@quicktextformatter.com',
});
...
export default function Terms() {
  ...
  return (
    <div className="min-h-screen flex flex-col" dir={dir}>
      <SeoHead
        title="Terms of Use"
        description="Review the Terms of Use for QuickTextFormatter."
        path="/terms"
        locale={locale}
      />
      <Navbar />
        ...
        <div className="mx-auto max-w-5xl space-y-8">
          <div className="space-y-4 text-center">
            <p className="text-sm font-medium uppercase tracking-wide text-indigo-600 dark:text-indigo-300">
              {legalContent.siteName} - {legalContent.termsTitle}
            </p>
            <h1 className="text-3xl md:text-4xl font-semibold">{legalContent.termsTitle}</h1>
          ...
          <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900 md:p-8">
            <div className={legalBodyClassName} dangerouslySetInnerHTML={{ __html: legalContent.termsHtml }} />
          </div>
        </div>
      </main>
      <Footer />
